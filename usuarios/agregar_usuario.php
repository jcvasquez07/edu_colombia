...
if (isset($_POST['guardar'])) {
    ...
    if (preg_match('/Duplicate entry/i', $error)) {
        $error = 'La direcci&#243;n de correo electr&#243;nico ya se encuentra en uso (probablemente el usuario ya existe en el sistema). Revise <a href="index.php?pagina=listado_usuarios&header=false" class="alert-link">el listado de usuarios</a>.';
    }
    $msg_usuario = 'Usuario: ' . $email . "<br />Clave: " . $clave_texto;
    echo $msg = ($resultado) ? '<div class="alert alert-success alert-dismissible fade show" role="alert"><strong>EL USUARIO SE AGREGO CORRECTAMENBTE. POR FAVOR TOME NOTA DE LA SIGUIENTE INFORMACION ANTES DE CERRAR ESTE MENSAJE<br />' . $msg_usuario . '</strong><button type="button" class="close" data-dismiss="alert" aria-label="Cerrar"><span aria-hidden="true">&times;</span></button></div>' : '<div class="alert alert-danger alert-dismissible fade show" role="alert">Error: el servidor dijo <strong>"' . $error . '"<strong><button type="button" class="close" data-dismiss="alert" aria-label="Cerrar"><span aria-hidden="true">&times;</span></button></div>';
}
?>
