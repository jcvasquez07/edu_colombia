...
if (isset($_POST['guardar'])) {
    $semestre = arreglar_texto(filter_input(INPUT_POST, 'semestre'));

    $error = "";
    $query = "INSERT INTO semestres SET semestre = '$semestre'";
    $resultado = $mysqli->query($query);
    if ($mysqli->error) {
        $error = "No se pudo insertar en la tabla semestres. El servidor dijo: " . htmlspecialchars($mysqli->error);
    }
...
if (isset($_POST['actualizar'])) {
    $id = filter_input(INPUT_POST, 'id_semestre');
    $semestre = arreglar_texto(filter_input(INPUT_POST, 'semestre'));
    $query = "UPDATE semestres SET semestre = '$semestre' WHERE id = '$id'";
    $resultado = $mysqli->query($query);
}
...
<div class="container-fluid">
    <form enctype="multipart/form-data" method="POST" action="">
        <div class="form-row">
            <div class="col-auto">
                <label for="semestre" class="sr-only">Nuevo semestre</label>
            </div>
            <div class="col-md-4">
                <input type="text" name="semestre" class="form-control" id="semestre" placeholder="Nueva semestre" required>
            </div>
    ...
    <table class="table table-hover" id="dt_listado">
        ...
        <tbody data-link="row" class="rowlink">
            <?php
            $query = "SELECT * FROM semestres WHERE 1 ORDER BY semestre";
            $resultado = $mysqli->query($query);
            $i = 0;
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalEditarLabel">Editar el semestre</h5>
                            ...
                            <div class="modal-body">
                                <form action="" method="POST">
                                    <input type="hidden" name="id_semestre" value="<?php echo $row['id']; ?>" />
                ...
                <div class="modal" id="modalBorrar<?php echo $row['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="modalBorrarTitle" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p>&#191;Esta seguro que desea borrar este semestre: <strong><?php echo $row['semestre']; ?>?</strong><p>
